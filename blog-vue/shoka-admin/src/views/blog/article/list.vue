<template>
  <div class="app-container">
    ...
    <el-row :gutter="24" style="color: #999;" class="mb15">
      ...
      <el-col :span="1.5" :class="isActive('all')" @click="changeStatus('all')">全部</el-col>
      <el-col :span="1.5" :class="isActive('public')" @click="changeStatus('public')">公开</el-col>
      <el-col :span="1.5" :class="isActive('secret')" @click="changeStatus('secret')">私密</el-col>
      <el-col :span="1.5" :class="isActive('draft')" @click="changeStatus('draft')">草稿</el-col>
      <el-col :span="1.5" :class="isActive('delete')" @click="changeStatus('delete')">回收站</el-col>
    </el-row>
    <!-- 搜索栏 -->
    <el-form :model="queryParams" :inline="true" v-show="showSearch">
      <el-form-item label="名称">
        <el-input @keyup.enter="handleQuery" v-model="queryParams.keyword" style="width: 150px" placeholder="请输入文章名称" clearable />
      </el-form-item>
      ...
      <el-form-item label="标签">
        <el-select v-model="queryParams.tagId" placeholder="请选择标签" clearable filterable @visible-change="getTag" style="width: 130px">
          <el-option v-for="item in tagList" :key="item.id" :label="item.tagName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="分类">
        <el-select v-model="queryParams.categoryId" placeholder="请选择分类" clearable filterable @visible-change="getCategory" style="width: 130px">
    ...
    <el-row :gutter="10" class="mb15">
      ...
      <el-col :span="1.5" v-if="queryParams.isDelete == 0">
        <el-button type="danger" plain icon="Delete" :disabled="articleIdList.length === 0" @click="handleRecycle(undefined)">批量回收</el-button>
      </el-col>
      <el-col :span="1.5" v-if="queryParams.isDelete == 1">
        <el-button type="success" plain icon="Finished" :disabled="articleIdList.length === 0" @click="handleRecycle(undefined)">批量恢复</el-button>
      </el-col>
    ...
    <el-table border :data="articleList" @selection-change="handleSelectionChange" v-loading="loading">
      ...
      <el-table-column prop="tagVOList" label="标签" width="160" align="center">
        <template #default="scope">
          <el-tag v-for="item of scope.row.tagVOList" :key="item.tagId" style="margin-right: 0.2rem; margin-top: 0.2rem">{{ item.tagName }}</el-tag>
        </template>
      </el-table-column>
      <!-- 文章浏览量 -->
      ...
      <el-table-column width="220" label="操作" align="center">
        <template #default="scope">
          <el-button type="primary" icon="Edit" link @click="handleEdit(scope.row.id)" v-if="scope.row.isDelete == 0">编辑</el-button>
          <el-button type="danger" icon="Delete" link @click="handleDelete(scope.row.id)">删除</el-button>
          <el-button type="danger" icon="Delete" link @click="handleRecycle(scope.row.id)" v-if="queryParams.isDelete == 0">回收</el-button>
          <el-button type="success" icon="Finished" link @click="handleRecycle(scope.row.id)" v-if="queryParams.isDelete == 1">恢复</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
    <pagination v-if="count > 0" :total="count" v-model:page="queryParams.current" v-model:limit="queryParams.size" @pagination="getList" />
  </div>

  <export-dialog title="导出文章列表" v-model="showExportDialog" @submit="batchExport" />
</template>
